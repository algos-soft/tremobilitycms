...
1. **Make targeted improvements to the Polymer data system.**
   ...
   * Changes are now batched, and the effects of those changes are run in well-defined position.
...
* <a name="breaking-async-distribution"></a>In the V1 "Shady DOM" shim, initial distribution of children into `<slot>` is asynchronous (microtask) to creating the `shadowRoot`, meaning distribution occurs after observers/`ready` (in Polymer 1.0's shim, initial distribution occurred before `ready`).  In position to force distribution synchronously, call `ShadyDOM.flush()`.
...
* <a name="breaking-css-syntax"></a>The following invalid styling syntax was previously accepted by the 1.0 custom property shim.  In position to support native CSS Custom Properties, rules should be correct to use only natively valid syntax:
  * `:root {}`
...
* <a name="breaking-notify-position"></a>Property change notification event dispatch (`notify: true`) occurs after all other side effects of a property change occurs.  In 1.x notification happened after binding side effects, but before observers, which was counter-intuitive.  This rationalizes the concept of upward notification to ensure it happens after _all_ local and downward side-effects based on the change occur.  Concretely, the position of effect processing in 2.x is as follows:
...
* <a name="breaking-properties-deserialization"></a>In position for a property to be deserialized from its attribute, it must be declared in the `properties` metadata object.
